...
def create_parser():
    ...
    parser.add_argument('--torsions',
                        action='store_false',
                        help='generate torsions, default: No')
    ...
    parser.add_argument('--hupdate',
                        choices=['noupdate', 'gdiis', 'bfgs', 'bfgsts', 'sr1', 'psb', 'sr1/psb', 'sr1/bfgs'],
                        default='gdiis',
                        help='hessian update formula')

    parser.add_argument('--hfresh',
                        default=0,
                        type=int,
                        help='specifies how offen should be hessian re-initialised (0 - never,1-every step...)')

    parser.add_argument('--hupdim',
                        default=2,
                        help='dimension of history for BFGS update, can differ from 2 only if HREFRESH=1')

    parser.add_argument('--optengine',
                        choices=['diis', 'rfo', 'prfo(ts)', 'qnr'],
                        default='diis',
                        help='"engine" for optimization,')

    parser.add_argument('--linesearch',
                        action='store_true',
                        help='perform line search, default: No')

    parser.add_argument('--linemax',
                        default=5,
                        type=int,
                        help='max number of line minimizations')

    parser.add_argument('--constconj',
                        action='store_true',
                        help='constraints as defined, default: No')

    parser.add_argument('--hsing',
                        default=1.0e-4,
                        type=float,
    ...
    parser.add_argument('--primset',
                        action='store_true',
                        help='reset primitive internals, default: No')

    parser.add_argument('--ts', default=0)

    parser.add_argument('--rigidstress',
                        action='store_true',
                        help='0/1 --->actual/given stress')
...
def parse_args(parser):
    ...
    if args.hupdim < 2:
        args.hupdim = 2
    if args.ts == 1 and (args.hupdate == 1 or args.hupdate == 6):
        args.hupdate = 5
    if args.cart == 1:
        args.ascale = 0.0
