...
class bmatrix:
        ...
        for i in range(len(coords)):
            ...
            if coords[i].dtyp == 'simple':
                if coords[i].tag == 'X':
                    ...
                    self.Bmatrix[i]=self.prepare_l(cartesian,coords[i],lattmat,invlat,BROW)
                elif coords[i].tag=='M':
                    self.Bmatrix[i]=self.prepare_m(cartesian,coords[i],lattmat,invlat,BROW)
                elif coords[i].tag=='RatioR':
                    self.Bmatrix[i]=self.prepare_ratior(cartesian,coords[i],lattmat,invlat,BROW)
                elif coords[i].tag=='A':
                    self.Bmatrix[i]=self.prepare_a(cartesian,coords[i],lattmat,invlat,BROW)
                elif coords[i].tag=='T':
                    self.Bmatrix[i]=self.prepare_dh(cartesian,coords[i],lattmat,invlat,BROW)
                elif coords[i].tag=='tV':
                    self.Bmatrix[i]=self.prepare_tv(cartesian,coords[i],lattmat,invlat,BROW)
                elif coords[i].tag=='IR1':
                    self.Bmatrix[i]=self.prepare_ir1(cartesian,coords[i],lattmat,invlat,BROW)
                elif coords[i].tag=='IR6':
                    self.Bmatrix[i]=self.prepare_ir6(cartesian,coords[i],lattmat,invlat,BROW)

                if relax:
                    if coords[i].tag == 'LR':
                        self.Bmatrix[i] = self.prepare_lr(coords[i], lattmat,
                                                          BROW)
                    elif coords[i].tag == 'LA':
                        self.Bmatrix[i] = self.prepare_la(coords[i], lattmat,
                                                          BROW)
                    elif coords[i].tag == 'LB':
                        self.Bmatrix[i] = self.prepare_lb(coords[i], lattmat,
                                                          BROW)
        #elif inttags[i]=='LS':
                    elif coords[i].tag == 'LV':
                        self.Bmatrix[i] = self.prepare_lv(lattmat, BROW)
                    elif coords[i].tag == 'RatioLR':
                        self.Bmatrix[i] = self.prepare_ratiolr(coords[i],
                                                               lattmat, BROW)

            if coords[i].dtyp == 'sum':
                self.Bmatrix[i] = self.prepare_sum(cartesian, coords[i],
                                                   lattmat, invlat, BROW)
            if coords[i].dtyp == 'norm':
                self.Bmatrix[i] = self.prepare_norm(cartesian, coords[i],
                                                    lattmat, invlat, BROW)
            if coords[i].dtyp == 'cn':
                self.Bmatrix[i] = self.prepare_cnum(cartesian, coords[i],
                                                    lattmat, invlat, BROW)
    ...
    def prepare_ratior(self, cartesian, coord, lattmat, invlat, BROW):
        ...
        coord_.what = tmp1[:2]
        coord_.where = tmp2[:2]
        dr1 = self.prepare_l(cartesian, coord_, lattmat, invlat,
                             np.zeros(len(BROW), dtype=float))
        coord_.what = tmp1[2:]
        coord_.where = tmp2[2:]
        dr2 = self.prepare_l(cartesian, coord_, lattmat, invlat,
                             np.zeros(len(BROW), dtype=float))
        coord.what = tmp1
        coord.where = tmp2
    ...
    def prepare_lnum(self, cartesian, lengths, intwhat, intwhere, lattmat,
                     derstep):
        """
    ...
    def calculate_da(self, a, b, c, d):
        ...
        fuck = sum(e12xe23 * e23xe34) / (np.linalg.norm(e12xe23) *
                                         np.linalg.norm(e23xe34))
        if fuck > 1:
            fuck = 1.0
        if fuck < -1:
            fuck = -1.0
    ...
    def prepare_sum(self, cartesian, coord, lattmat, invlat, BROW):
        ...
        for i in range(len(coord.tag)):
            XROW = np.zeros(len(BROW), dtype=float)
            tcoord = datastruct.Complextype('simple', [1], coord.tag[i],
                                            coord.what[i], [None],
                                            coord.where[i], 0.0, 'free')
            if coord.tag[i] == 'X':
                BROW=BROW+coord.coefs[i]*self.prepare_sing(cartesian,tcoord,0,lattmat,invlat,XROW)
            if coord.tag[i] == 'Y':
                BROW=BROW+coord.coefs[i]*self.prepare_sing(cartesian,tcoord,1,lattmat,invlat,XROW)
            if coord.tag[i] == 'Z':
                BROW=BROW+coord.coefs[i]*self.prepare_sing(cartesian,tcoord,2,lattmat,invlat,XROW)
            if coord.tag[i] == 'fX':
            ...
            if coord.tag[i] == 'M':
                BROW=BROW+coord.coefs[i]*self.prepare_m(cartesian,tcoord,lattmat,invlat,XROW)
            if coord.tag[i] == 'A':
            ...
            if coord.tag[i] == 'RatioR':
                ...
                BROW=BROW+coord.coefs[i]*self.prepare_ratior(cartesian,tcoord,lattmat,invlat,XROW)
            if coord.tag[i] == 'LR':
            ...
            if coord.tag[i] == 'RatioLR':
                BROW=BROW+coord.coefs[i]*self.prepare_ratiolr(tcoord,lattmat,XROW)
            if coord.tag[i] == 'hX':
        ...
        for i in range(len(coord.tag)):
            ...
            if coord.tag[i] == 'X':
                ...
                BROW = BROW + (coord.coefs[i]**2) * dist * \
                    self.prepare_sing(cartesian, tcoord, 0, lattmat, invlat,
                                      BROW)
            if coord.tag[i] == 'Y':
                dist = cartesian[tcoord.what[0]][1]
                complexcoord += ((coord.coefs[i]**2) * dist)
                BROW = BROW + (coord.coefs[i]**2) * dist * \
                    self.prepare_sing(cartesian, tcoord, 1, lattmat, invlat,
                                      BROW)
            if coord.tag[i] == 'Z':
                dist = cartesian[tcoord.what[0]][2]
                complexcoord += ((coord.coefs[i]**2) * dist)
                BROW = BROW + (coord.coefs[i]**2) * dist * \
                    self.prepare_sing(cartesian, tcoord, 2, lattmat, invlat,
                                      BROW)
            if coord.tag[i] == 'R':
                a = cartesian[tcoord.what[0]] + np.dot(tcoord.where[0],
                                                       lattmat)
                b = cartesian[tcoord.what[1]] + np.dot(tcoord.where[1],
                                                       lattmat)
                vector = a - b
                dist = np.linalg.norm(vector)
                complexcoord += ((coord.coefs[i]**2) * dist)
                BROW = BROW + (coord.coefs[i]**2) * dist * \
                    self.prepare_l(cartesian, tcoord, lattmat, invlat,
                                   np.zeros(len(BROW), dtype=float))
            if coord.tag[i] == 'A':
                a = cartesian[tcoord.what[0]] + np.dot(tcoord.where[0],
                                                       lattmat)
                v = cartesian[tcoord.what[1]] + np.dot(tcoord.where[1],
                                                       lattmat)
                b = cartesian[tcoord.what[2]] + np.dot(tcoord.where[2],
                                                       lattmat)
                diffav = a - v
                diffbv = b - v
                d1 = np.linalg.norm(diffav)
                ...
                BROW = BROW + (coord.coefs[i]**2) * alpha * \
                    self.prepare_a(cartesian, tcoord, lattmat, invlat,
                                   np.zeros(len(BROW), dtype=float))
            if coord.tag[i] == 'T':
                a = cartesian[tcoord.what[0]] + np.dot(tcoord.where[0],
                                                       lattmat)
                b = cartesian[tcoord.what[1]] + np.dot(tcoord.where[1],
                                                       lattmat)
                c = cartesian[tcoord.what[2]] + np.dot(tcoord.where[2],
                                                       lattmat)
                d = cartesian[tcoord.what[3]] + np.dot(tcoord.where[3],
                                                       lattmat)
                vector1 = a - b
                vector2 = b - c
                vector3 = c - d
                ...
                BROW = BROW + (coord.coefs[i]**2) * dangle * \
                    self.prepare_dh(cartesian, tcoord, lattmat, invlat,
                                    np.zeros(len(BROW), dtype=float))
        ...
        for i in range(len(coord.tag)):
            if coord.tag[i] == 'R':
                if abs(coord.coefs[i]) > 1e-4:
                    tcoord = datastruct.Complextype('simple', [1],
                                                    coord.tag[i],
                                                    coord.what[i], [None],
                                                    coord.where[i], 0.0,
                                                    'free')
                    ...
                    BROW_ = self.prepare_l(cartesian, tcoord, lattmat, invlat,
                                           np.zeros(len(BROW), dtype=float))
