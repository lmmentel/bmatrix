...
class bmatrix:
 ...
 def __init__(self,cartesian,coords,numofatoms,lattmat,RELAX):
   ...
   self.Bmatrix=zeros((dimdim1,dimdim2),Float)
   derstep=0.00001
 ...
 def calculate_da(self,a,b,c,d):
   ...
   if fuck<-1: fuck=-1.0
   dangle=acos(fuck)
   if sum(e12xe23*e34)>=0: dangle=-dangle
   return dangle
 ...
 def put_intocell(self,dmat):
   for i in range(3):
     if dmat[0][i]<0.0: dmat[0][i]+=1
     if dmat[0][i]>=1.0: dmat[0][i]-=1
   return dmat
 ...
 def prepare_norm(self,cartesian,coord,lattmat,invlat,BROW):
   ...
   for i in range(len(coord.tag)):
     ...
     if coord.tag[i]=='X':
       ...
       BROW=BROW+(coord.coefs[i]**2)*dist*self.prepare_sing(cartesian,tcoord,0,lattmat,invlat,BROW)
     if coord.tag[i]=='Y':
       dist=cartesian[tcoord.what[0]][1]
       complexcoord+=((coord.coefs[i]**2)*dist)
       BROW=BROW+(coord.coefs[i]**2)*dist*self.prepare_sing(cartesian,tcoord,1,lattmat,invlat,BROW)
     if coord.tag[i]=='Z':
